    ...
    for (vector<gate>::size_type uu1 = 0; uu1 < gates.size(); uu1++) {
        for (vector<wire>::size_type uu2 = 0; uu2 < gates[uu1].inputs.size(); uu2++) {
            ...
            if (build_func == 0) {
                ...
                gates[uu1].inputs[uu2].w_func.push_back("#");
                gates[uu1].inputs[uu2].w_func.push_back("-");
                gates[uu1].inputs[uu2].cont_val.push_back(1.00);		// Initializing Control Values of Primary Inputs
            }
        }
    }
...
bool encoder::parse_calc (deque<string> function, deque<func_in> func_inputs) {
   ...
   // deque<oprt_type> operate;						// Container for major operators (AND or OR) of the function.
    ...
    deque<func_in>::size_type inp_iter1 = 0;

    deque<func_in>::size_type inp_iter2 = 0;
    ...
    for (deque<split>::size_type ww1 = 0; ww1 < splitter.size(); ww1++) {
        ...
        if (splitter[ww1].sect[0] != "YEK") {
        	for (inp_iter1 = 0; inp_iter1 < func_inputs.size(); inp_iter1++) {
            	if (splitter[ww1].sect[0] == func_inputs[inp_iter1].in_name) {
            		break;
            	}
            }
        	if (splitter[ww1].sect[1] == "-") {
                temp_res = func_inputs[inp_iter1].in_val;
            }
        ...
        if (splitter[ww1].sect[3] != "YEK") {
        	for (inp_iter2 = 0; inp_iter2 < func_inputs.size(); inp_iter2++) {
        	   if (splitter[ww1].sect[3] == func_inputs[inp_iter2].in_name) {
        	       break;
        	   }
        	}
        	if (splitter[ww1].sect[4] == "-") {
                if (splitter[ww1].sect[2] == "+") {
                    temp_res = temp_res | func_inputs[inp_iter2].in_val;
                }
...
int main () {
    ...
    double epsilon = 2.50;												// ******** IMPORTANT: Threshold for Control Values ********
    ...
    for (deque<string>::size_type mmm = 0; mmm < circuit_gates[0].outputs[0].w_func.size(); mmm++) {
    	cout << circuit_gates[0].outputs[0].w_func[mmm];
    }

    cout << endl;

    for (vector<bool>::size_type bbn = 0; bbn < res0_vect.size(); bbn++) {
    	cout << res0_vect[bbn] << " - ";
    }

    cout << endl;
    for (vector<split>::size_type tin1  = 0; tin1 < splitter.size(); tin1++) {
    	//for (deque<string>::size_type tin2 = 0; tin2 < splitter[tin1].sect.size(); tin2++) {
    		cout << splitter[tin1].sign;
    	//}
    	cout << endl;
    }

    //cout << endl << splitter[1].sect[3];

    cout << endl;

    for (deque<oprt_type>::size_type kkk = 0; kkk < operate.size(); kkk++) {
    	cout << operate[kkk].oprt << " --- " << operate[kkk].sign << endl;
    }

    for (vector<bool>::size_type uuu = 0; uuu < vect_results.size(); uuu++) {
    	cout << vect_results[uuu] << " -- ";
    }
