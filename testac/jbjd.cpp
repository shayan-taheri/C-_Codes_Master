...
public:
 	...
 	unsigned long long int inputs_number[2];								// Number of Inputs on Wire
};
...
void comp_gates::AND_Base (wire& A, wire& B, wire& C) {
	...
	if (A.inputs_number[0] == 0 && A.inputs_number[1] == 0) {
		C.inputs_number[0] = 1;
	}
	else {
		C.inputs_number[0] = A.inputs_number[0] + A.inputs_number[1];
	}


	if (B.inputs_number[0] == 0 && B.inputs_number[1] == 0) {
		C.inputs_number[1] = 1;
	}
	else {
		C.inputs_number[1] = B.inputs_number[0] + B.inputs_number[1];
	}
...
int main () {
    ...
    cout << endl << temp_oprt_signs << endl;

    for (deque<oprt_type>::size_type deq1 = 0; deq1 < operate.size(); deq1++) {
    	cout << operate[deq1].oprt << " --> " << operate[deq1].sign << endl;
    }

    for (deque<split>::size_type spp1 = 0; spp1 < splitter.size(); spp1++) {
    	for (deque<string>::size_type spp2 = 0; spp2 < splitter[spp1].sect.size(); spp2++) {
        	cout << splitter[spp1].sect[spp2];
    	}
    	cout << " ---> " << splitter[spp1].sign;
    	cout << endl;
    }

    cout << endl;
    for (deque<string>::size_type uuy = 0; uuy < circuit_gates[0].outputs[0].w_func.size(); uuy++) {
    	cout << circuit_gates[0].outputs[0].w_func[uuy];
    }
