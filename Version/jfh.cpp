...
bool encoder::parse_calc (deque<string> function, deque<func_in> func_inputs) {
    ...
    for (deque < deque<bool> >::size_type resc1 = 0; resc1 < clust_results.size(); resc1++) {
    	...
    	for (deque<bool>::size_type resc2 = 0; resc2 < clust_results[resc1].size(); resc2++) {
    		...
    		if ((resc2+1) < clust_results[resc1].size()+1) {resc2++;}
        }
    	clust_finals.push_back(final_result);
    }

    deque_results.clear();
    //for (deque < deque<oprt_type> >::size_type clst3 = 0; clst3 < clust_oprt.size(); clst3++) {
    //	deque_results.push_back(clust_results[clst3][0]);
    //}
...
int main () {
    ...
    bool check_ave = 0;													// ******** IMPORTANT: Condition of Average Procedure ********

    bool check_med = 1;													// ******** IMPORTANT: Condition of Median Procedure ********
    ...
    cout << endl << splitter[3].sect[0] << " -- " << splitter[3].sign << " -- " << splitter[3].value << endl;

    //cout << endl << clust_oprt.size();

    cout << endl << res0_vect[1];

    cout << endl;

    //for (unsigned long pppx = 0; pppx < circuit_gates[0].inputs[0].w_func.size(); pppx++) {
    //	cout << circuit_gates[0].inputs[0].w_func[pppx];
    //}
