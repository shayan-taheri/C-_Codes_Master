...
deque < deque<bool> > clust_results;
...
bool encoder::parse_calc (deque<string> function, deque<func_in> func_inputs) {
    ...
    /*for (deque<oprt_type>::size_type maj1 = 0; (maj1 < (major_oprt.size()+1)) && (deque_results.size() != 1); maj1++) {
    	final_result = 0;
    	if ((maj1+1) < major_oprt.size()+1) {
    		final_result = deque_results[maj1] | deque_results[maj1+1];
    	}
    	if (major_oprt[maj1].sign == "!") {
    		final_result = !final_result;
    	}
    	major_oprt.pop_front();
    	deque_results.pop_front();
    	deque_results.pop_front();
    	deque_results.push_back(final_result);
    }

    if (deque_results.size() != 1) { cerr << "ERROR!: Problem in calculating final results" << endl; }

    else if (deque_results.size() == 1) {
    	final_result = deque_results[0];
    }*/
...
std::vector<double> encoder::control_function (deque<string> FUNCTION) {
    ...
    for (deque<string>::size_type ptr = 0; ptr < FUNC_INS.size(); ptr++) {	// FUNC_INS.size()
        ...
        FUNC_INS[ptr].in_val = 1;		// FUNC_INS[ptr].in_val = 1     for (unsigned long long tab1_indx = 0; tab1_indx < table[0].size(); tab1_indx++) {
        for (unsigned long long tab1_indx = 1; tab1_indx < 2; tab1_indx++) {
            temp_value = 0;
            if (ptr == 0) {
...
int main () {
    ...
    /////// MAHAL BARRESI RESULT /////////

    cout << endl << deque_results[0] << " --- " << deque_results[1] << endl;

    cout << endl << splitter[0].sect[0] << " -- " << splitter[0].sign << " -- " << splitter[0].value << endl;

    cout << endl << splitter[1].sect[0] << " -- " << splitter[1].sign << " -- " << splitter[1].value << endl;

    cout << endl << splitter[2].sect[0] << " -- " << splitter[2].sign << " -- " << splitter[2].value << endl;

    cout << endl << splitter[4].sect[0] << " -- " << splitter[3].sign << " -- " << splitter[3].value << endl;

    //cout << endl << clust_oprt.size();

    cout << endl << "vect: " << res1_vect[3];

    cout << endl << endl << "final: " << final_result << endl;

    for (unsigned long pppx = 0; pppx < circuit_gates[0].outputs[0].w_func.size(); pppx++) {
    	cout << circuit_gates[0].outputs[0].w_func[pppx];
    }

    cout << endl;
    for (deque<oprt_type>::size_type bbn1 = 0; bbn1 < operate.size(); bbn1++) {
    	cout << operate[bbn1].oprt << " --- " << operate[bbn1].sign << endl;
    }

    cout << endl << endl;

    cout << clust_results.size() << endl;

    cout << "results: " << clust_results[2][0];

    cout << endl << FUNC_INS.size();
